    ...
    public class CommandRepository : ICommandRepository
    {
        ...
        public async Task<int> RedeemCoupon(RedemptCoupon coupon, int uid)
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    await connection.ExecuteAsync(query2, new { cid = coupon.cpnId, uid = uid, op = (int) Operation.Redeem, time = DateTime.Now.ToString(), cname = coupon.ClientName, cpos = coupon.ClientPos }, transaction);
        ...
        public async Task<int> VoidCoupon(RedemptCoupon coupon, int uid)
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                ...
                {
                    result = await connection.QueryFirstAsync<int>(query, new { couponid = coupon.cpnId, amount = coupon.amount }, transaction);
                    await connection.ExecuteAsync(query2, new { cid = coupon.cpnId, uid = uid, op = (int)Operation.Void, time = DateTime.Now.ToString(), cname = coupon.ClientName, cpos = coupon.ClientPos }, transaction);
        ...
        public async Task<int> ChangeStatus(StatusCoupon coupon, int uid)
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    result = await connection.ExecuteAsync(query2, new { cid = coupon.cpnId, uid = uid, op = (int)op, time = DateTime.Now.ToString(), cname = coupon.ClientName, cpos = coupon.ClientPos }, transaction);
                    if (result < 1)
                    {
                        throw new Exception();
                    }
